...
```{r, eval=FALSE}
# Observed DV by group: summary table
data_long %>% 
...
Plot the data to eyeball the potential effect.  Remember the center line in each box represents the median, not the mean. 

```{r, eval=FALSE}
# Observed DV by group: boxplots
data_long %>% 
...
```{r, eval=FALSE}
# Observed DV by group: Mean-SD plots
data_long %>% 
  ggplot(aes(x = group_IV,
             y = continuous_DV)) + 
  stat_summary() +
...
```{r, eval=FALSE}
# Observed DV by group: violin-boxplot w/means
data_long %>% 
